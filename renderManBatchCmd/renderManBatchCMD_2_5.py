    ...
    def modpushButton_submitDeadLine(self):
      ...
      #  batchJobJsonFile = open(commandsfileName,'w')
        ...
        if self.radioButton_single.isChecked() == True:
            ...
            #popCommand = jobSendToDeadLineBatchFileName
            #popCommand=['deadlinecommand.exe',jobInfoFileName,pluginInfoFileName,commandsfileName]
            #popCommand.append(jobInfoFileName)
            #popCommand.append(pluginInfoFileName)
            #popCommand.append(commandsfileName)
            
            # print jobSendToDeadLineBatchFileName
            #popCommand = ['deadlinecommand.exe', u'//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223152333/jobInfo.job', u'//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223152333/pluginInfo.job', u'//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223152333/commandsfile.txt ']

           # for i in jobSendToDeadLineBatchFileName:
            #popCommandB = 'deadlinecommand.exe %s %s %s'%(jobInfoFileName,pluginInfoFileName,commandsfileName)
            #popCommand = popCommandB.encode('raw_unicode_escape').decode('utf8')
           # print popCommand[1].encode('raw_unicode_escape').decode('utf8')
            #print popCommand.encode('raw_unicode_escape').decode('utf8')
            #sendJob = subprocess.Popen(popCommand,stdout=subprocess.PIPE,shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo
            #sendJob =subprocess.call()
           # sendJob = subprocess.Popen(popCommand,stdout=subprocess.PIPE,universal_newlines=True, shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo
           # sendJob = subprocess.Popen(popCommand,stdout=subprocess.PIPE,universal_newlines=True, shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo
           
           
            #aa = '//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223145242/sendToDeadLine.bat'
            #cc = open(aa,'r')
            #d = cc.read()
            #print d
            ...
            #sendJob = subprocess.Popen(['deadlinecommand.exe','%s','%s','%s'%(jobInfoFileName,pluginInfoFileName,commandsfileName)],stdout=subprocess.PIPE,shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo
            
            #os.system('deadlinecommand.exe %s %s %s' '%(jobInfoFileName,pluginInfoFileName,commandsfileName)')
            #os.system(popCommand)
            ##print sendJob
        


    def modpushButton_submitDeadLine3(self):                       
       # cwdLocation = '\\\\mcd-server\\art_3d_project\\fish_battle_v02_cf\\shot\\vr_test\\layout\\renderCmd\\0223131458'#self.cmdFileLocation + " "
       # cwdLocation = '\''+'//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223131458'+'\''#self.cmdFileLocation + " "
       # cwdLocation = "c/temp"
      #  //mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223135008
        jobInfoFileName = '//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223151146/jobInfo.job'
        pluginInfoFileName = '//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223151146/pluginInfo.job'
        commandsfileName = '//mcd-server/art_3d_project/fish_battle_v02_cf/shot/vr_test/layout/renderCmd/0223151146/commandsfile.txt' 
       # print cwdLocation
        #a= "help"

        popCommand=['deadlinecommand.exe']
        popCommand.append(jobInfoFileName)
        popCommand.append(pluginInfoFileName)
        popCommand.append(commandsfileName)
        
        #popCommand.append(cwdLocation)
        #popCommand.append('cwd='+'\''+'%s'%cwdLocation+'\'')
       # print popCommand
            
        sendJob = subprocess.Popen(popCommand,stdout=subprocess.PIPE,shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo

           # sendJob = subprocess.Popen(["deadlinecommand.exe","help"], stdout=subprocess.PIPE, shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo
                                    #  (["dir"], stdout=subprocess.PIPE, shell=True).communicate()[0]
           # sendJob = subprocess.Popen(["dir"], stdout=subprocess.PIPE, shell=True).communicate()[0]  #cmd run deadlineCommand -Pools,that get pools from deadline Repo

           # sendJob = subprocess.Popen(jobSendToDeadLineBatchFileName, shell=True, stdout = subprocess.PIPE)
