...
class mod_MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def modlistWidget_assetProj(self):
       ...
       # print self.listWidget_assetProj
        item = self.listWidget_assetProj.currentItem()
        self.assetSelect = item.text()
        print self.assetSelect
       # self.getAssetTreeDir(self.assetSelect)
        if self.topLevelType == "1":
            print"aaaaaaaaa"
            self.classKey = self.assetsDict[self.assetSelect]

            #branchTreeDict = self.projectDict['assets'][self.classKey][self.assetSelect]
            try:
        ...
        elif self.topLevelType == "2":
            print "bbbbbbbbbbb"
           # branchTreeDict = self.projectDict['shot'][self.assetSelect]
            try:
    ...
    def projectAssembleDefine(self):
        self.topLevelItem = ['assets','shot','global','publish']
    ...
    def getProjInfo(self):
        ...
        self.root="//mcd-server/art_3d_project/" + self.projectName
        
        basePath = ['assets','shot','global','publish','QC','reference','output']
        basePathList = []

        for baseItem in basePath:
