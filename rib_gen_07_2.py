...
#-----------------------------------------signalbegin-------------------------------------------------------------------
        ...
        #GPUCache connect to file control
        self.pushButton_breakGPU.clicked.connect(self.modpushButton_breakGPU)
        self.pushButton_connectGPU.clicked.connect(self.modpushButton_connectGPU)
...
#------------------------------------------GPUCache store and connect---------------------------------------

    def modpushButton_breakGPU(self):
        print "store GPUCacne linking path and break connection begining"
        
      #  print "let it go"
        

        projectPath = self.currentProj + 'data/cache/' 
            
        gpuCacheObj = pm.ls(type="gpuCache")

        gpuCacheDict = {}

        for gpuNode in gpuCacheObj:
            filePath = pm.getAttr("%s.cacheFileName"%gpuNode)
            gpuCacheDict.update({gpuNode.name():filePath})
        ...
        for file in os.listdir(projectPath):
            if len(file) == 26:
                #print file        
                if file[0:16] == "gpuCacheMetaDate" and file [-4:] == "json":
                  #  print file
                                
                    fileSplit = file.split(".")
                    jsonFileNameTitle =fileSplit[0]
                    jsonFileNameCount = fileSplit[1]
                    jsonFileNameExt = fileSplit[2]
                    fileCountList.append(int(jsonFileNameCount))
                    
                    
                    
        if len(fileCountList) == 0:
            outputJsonName = "gpuCacheMetaDate.0001.json"
            print outputJsonName

                    
        else:
            fileCount = "%#04d"%(fileCountList[-1]+ 1)
            outputJsonName = "gpuCacheMetaDate."+ fileCount + ".json"
            print outputJsonName
                        
        
        storeFileName = projectPath +  outputJsonName  ##data/cache in project path 
                    
        storeGpu = open(storeFileName,'w')
        

        storeGpu.write(json.dumps(gpuCacheDict, sort_keys=True , indent =4))
        storeGpu.close()
        
        print "brake GPU connection"
        ...
        print "store GPUCacne linking path and break connection finish"
        

        
    def modpushButton_connectGPU(self):
        ...
        if len(fileCountList) == 0:
            print "No GPUCacheList"
                
                    
        else:
            fileCount = "%#04d"%(fileCountList[-1])
            outputJsonName = "gpuCacheMetaDate."+ fileCount + ".json"
            #print outputJsonName
            storeFileName = projectPath + outputJsonName
            print storeFileName
            GPUCacheJsonFile = open(storeFileName)
           # print GPUCacheJsonFile
            readGPUCacheJsonFile = json.load(GPUCacheJsonFile)
            
           # print readGPUCacheJsonFile
           # print type(repr(readGPUCacheJsonFile))
            for gpuNode in readGPUCacheJsonFile:
                #print readGPUCacheJsonFile[gpuNode]
                try:
                    pm.setAttr("%s.cacheFileName"%gpuNode,readGPUCacheJsonFile[gpuNode])
                except:
                    pass
